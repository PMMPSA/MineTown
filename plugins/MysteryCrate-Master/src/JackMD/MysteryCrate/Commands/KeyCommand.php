...
class KeyCommand extends PluginCommand{
	...
	public function __construct(Main $plugin){
		...
		$this->setUsage("§l§f[§eCrate§f]§b /key [type] [player] [amount]");
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($plugin instanceof Main){
			if(!isset($args[0])){
				$sender->sendMessage(TextFormat::RED . "§l§f[§eCrate§f]§b Usage: /key [type] [player] [amount]");
				return true;
			}
			$target = $sender;
			...
			if(isset($args[1])){
				...
				if(!$target instanceof Player){
					$sender->sendMessage(TextFormat::RED . "§l§f[§eCrate§f]§b Invalid player.");
					return true;
				}
			}else{
				if(!$target instanceof Player){
					$sender->sendMessage(TextFormat::RED . "§l§f[§eCrate§f]§b Please specify a player.");
					return true;
				}
			}
			if(!$plugin->getCrateType($args[0])){
				$sender->sendMessage(TextFormat::RED . "§l§f[§eCrate§f]§b Invalid crate type.");
				return true;
			}
			...
			$sender->sendMessage(TextFormat::GREEN . ucfirst($args[0]) . " Key đã được cho.");
			return true;
		}
		return true;
	}
