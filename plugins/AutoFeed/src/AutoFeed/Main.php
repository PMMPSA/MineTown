...
class Main extends PluginBase implements Listener{
	...
	{
		$this->getLogger()->info("AutoFeed code by Lòn is working");
	...
	public function onCommand(CommandSender $sender, Command $command, string $label, array $params) : bool
	{
		...
		if($command->getName() == "autofeed"){
			if(isset($this->players[$username])){
				...
				$sender->sendMessage("§a§l• §eMine§bTown§a • §cBạn đã tắt chế độ tự động ăn");
				return true;
			}else{
				$this->players[$username] = true;
				$sender->sendMessage("§a§l• §eMine§bTown§a • §aBạn đã bật chế độ tự động ăn");
				return true;
			}
		}else{
			return false;
