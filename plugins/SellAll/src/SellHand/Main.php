			...
			if(!($sender instanceof Player)){
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(isset($args[0]) && strtolower($args[0]) == "hand"){
						...
						if($this->skillminer->get()->richdreamer[strtolower($sender->getName())] === "on"){
...
if($this->skillminer->get()->richdreamer[strtolower($sender->getName())] === "on"){
	EconomyAPI::getInstance()->addMoney($sender, ($this->sell->get($itemId) * $item->getCount()) * 3);
						...
						EconomyAPI::getInstance()->addMoney($sender, $this->sell->get($itemId) * $item->getCount());
								...
								$sender->sendMessage(TF::GREEN . TF::BOLD . "§l§e♦§aMine§cTown§e♦§r " . TF::RESET . TF::GREEN . "Đã bán " . TF::RED . "" . $price . TF::GREEN . " xu ( " . $item->getCount() . " " . $item->getName() . " mỗi cái " . $this->sell->get($item->getId()) . " xu ).");
								if($item->getCustomName() == ""){
						...
						$sender->sendMessage(TF::RED . TF::BOLD . "(!) " . TF::RESET . TF::GRAY . "404 ×.× > Nothing here :(");
					}else{
						$sender->sendMessage(TF::RED . TF::BOLD . "(!) " . TF::RESET . TF::DARK_RED . "Cửa hàng vật phẩm");
						$sender->sendMessage(TF::RED . "- " . TF::DARK_RED . "/sell all " . TF::GRAY . "- bán tất cả mọi thứ trong túi đồ của bạn");
						return true;
					}
				}else{
