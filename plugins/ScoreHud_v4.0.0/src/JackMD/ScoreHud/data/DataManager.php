...
class DataManager{
	 ...
	 * @return float|string
	 */
	public function getPlayerLevelPlot(Player $player){
		/** @var LevelPlot $levelplot */
		$levelplot = $this->plugin->getServer()->getPluginManager()->getPlugin("LevelPlot");
		if($levelplot instanceof LevelPlot){
			return $levelplot->getLevelPlot($player);
		}else{
	...
	public function getPlayerPoint(Player $player){
		...
		if($pointapi instanceof PointAPI){
			return $pointapi->myPoint($player);
		}else{
			return "Plugin not found";
		}
	}
