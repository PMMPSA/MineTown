            ...
            $item = $sender->getInventory()->getItemInHand();
				      if($item instanceof Armor or $item instanceof Tool){
				        $id = $item->getId();
					      ...
					      $sender->sendMessage("§f§l[§cFix§f]§r§a Vật Phẩm§6.". $item->getName() . "§a đã được sửa chữa với giá§6 $cash ");
					      return true;
					    } else {
				        	$sender->sendMessage("§f§l[§cFix§f]§r§c Vật phẩm trên tay phải là công cụ hoặc giáp!");
					        return false;
					    }
            return true;
          } else {
            $sender->sendMessage("§f§l[§cFix§f]§r§c Bạn không đủ§6 $cash §axu để sửa chữa");
            return true;
          }
        }
    }
