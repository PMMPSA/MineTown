...
namespace onebone\rebirthcoinapi\task;
...
class SortTask extends AsyncTask{
	 ...
	 * @param array				$banList
	 */
	public function __construct(string $sender, array $rebirthcoinData, bool $addOp, int $page, array $ops, array $banList){
		$this->sender = $sender;
		$this->rebirthcoinData = $rebirthcoinData;
		$this->addOp = $addOp;
		$this->page = $page;
		$this->ops = $ops;
	...
	private function getTopList(){
		$toprebirthcoin = (array)$this->rebirthcoinData;
		$banList = (array)$this->banList;
		$ops = (array)$this->ops;
		arsort($toprebirthcoin);

		$ret = [];

		$n = 1;
		$this->max = ceil((count($toprebirthcoin) - count($banList) - ($this->addOp ? 0 : count($ops))) / 5);
		$this->page = (int)min($this->max, max(1, $this->page));

		foreach($toprebirthcoin as $p => $m){
			$p = strtolower($p);
	...
	public function onCompletion(Server $server){
		if($this->sender === "CONSOLE" or ($player = $server->getPlayerExact($this->sender)) instanceof Player){ // TODO: Rcon
			$plugin = RebirthCoinAPI::getInstance();

			$output = ($plugin->getMessage("toprebirthcoin-tag", [$this->page, $this->max], $this->sender)."\n");
			$message = ($plugin->getMessage("toprebirthcoin-format", [], $this->sender)."\n");
