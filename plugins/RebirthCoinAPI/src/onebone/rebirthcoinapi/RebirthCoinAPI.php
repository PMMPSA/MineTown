...
namespace onebone\rebirthcoinapi;
...
use onebone\rebirthcoinapi\provider\Provider;
use onebone\rebirthcoinapi\provider\YamlProvider;
use onebone\rebirthcoinapi\provider\MySQLProvider;
use onebone\rebirthcoinapi\event\rebirthcoin\SetRebirthCoinEvent;
use onebone\rebirthcoinapi\event\rebirthcoin\ReduceRebirthCoinEvent;
use onebone\rebirthcoinapi\event\rebirthcoin\AddRebirthCoinEvent;
use onebone\rebirthcoinapi\event\rebirthcoin\RebirthCoinChangedEvent;
use onebone\rebirthcoinapi\event\account\CreateAccountEvent;
use onebone\rebirthcoinapi\task\SaveTask;

class RebirthCoinAPI extends PluginBase implements Listener{
	const API_VERSION = 3;
	 ...
	 * @return array
	 */
	public function getAllRebirthCoin() : array{
	 ...
	 * @return bool
	 */
	public function createAccount($player, $defaultRebirthCoin = false, bool $force = false) : bool{
		...
		if(!$this->provider->accountExists($player)){
			$defaultRebirthCoin = ($defaultRebirthCoin === false) ? $this->getConfig()->get("default-rebirthcoin") : $defaultRebirthCoin;

			$this->getServer()->getPluginManager()->callEvent($ev = new CreateAccountEvent($this, $player, $defaultRebirthCoin, "none"));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->createAccount($player, $ev->getDefaultRebirthCoin());
			}
		}
		return false;
	}
	 ...
	 * @return float|bool
	 */
	public function myRebirthCoin($player){
		return $this->provider->getRebirthCoin($player);
	}
	 ...
	 * @return int
	 */
	public function setRebirthCoin($player, $amount, bool $force = false, string $issuer = "none") : int{
		if($amount < 0){
		...
		if($this->provider->accountExists($player)){
			...
			if($amount > $this->getConfig()->get("max-rebirthcoin")){
				return self::RET_INVALID;
			}

			$this->getServer()->getPluginManager()->callEvent($ev = new SetRebirthCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->setRebirthCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new RebirthCoinChangedEvent($this, $player, $amount, $issuer));
	 ...
	 * @return int
	 */
	public function addRebirthCoin($player, $amount, bool $force = false, $issuer = "none") : int{
		if($amount < 0){
			return self::RET_INVALID;
		}
		if($player instanceof Player){
		...
		$player = strtolower($player);
		if(($rebirthcoin = $this->provider->getRebirthCoin($player)) !== false){
			$amount = round($amount, 2);
			if($rebirthcoin + $amount > $this->getConfig()->get("max-rebirthcoin")){
				return self::RET_INVALID;
			}

			$this->getServer()->getPluginManager()->callEvent($ev = new AddRebirthCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->addRebirthCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new RebirthCoinChangedEvent($this, $player, $amount + $rebirthcoin, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	 ...
	 * @return int
	 */
	public function reduceRebirthCoin($player, $amount, bool $force = false, $issuer = "none") : int{
		if($amount < 0){
			return self::RET_INVALID;
		}
		if($player instanceof Player){
		...
		if(($rebirthcoin = $this->provider->getRebirthCoin($player)) !== false){
			$amount = round($amount, 2);
			if($rebirthcoin - $amount < 0){
				return self::RET_INVALID;
			}

			$this->getServer()->getPluginManager()->callEvent($ev = new ReduceRebirthCoinEvent($this, $player, $amount, $issuer));
			if(!$ev->isCancelled() or $force === true){
				$this->provider->reduceRebirthCoin($player, $amount);
				$this->getServer()->getPluginManager()->callEvent(new RebirthCoinChangedEvent($this, $player, $rebirthcoin - $amount, $issuer));
				return self::RET_SUCCESS;
			}
			return self::RET_CANCELLED;
		}
	...
	private function registerCommands(){
		...
		$commands = [
			"myrbcoin" => "\\onebone\\rebirthcoinapi\\command\\MyRebirthCoinCommand",
			"toprbcoin" => "\\onebone\\rebirthcoinapi\\command\\TopRebirthCoinCommand",
			"setrbcoin" => "\\onebone\\rebirthcoinapi\\command\\SetRebirthCoinCommand",
			"seerbcoin" => "\\onebone\\rebirthcoinapi\\command\\SeeRebirthCoinCommand",
			"giverbcoin" => "\\onebone\\rebirthcoinapi\\command\\GiveRebirthCoinCommand",
			"takerbcoin" => "\\onebone\\rebirthcoinapi\\command\\TakeRebirthCoinCommand",
			"payrbcoin" => "\\onebone\\rebirthcoinapi\\command\\PayRebirthCoinCommand",
			"setlangrbcoin" => "\\onebone\\rebirthcoinapi\\command\\SetLangCommand",
			"mystatusrbcoin" => "\\onebone\\rebirthcoinapi\\command\\MyStatusRebirthCoinCommand"
		];
		foreach($commands as $cmd => $class){
			$map->register("rebirthcoinapi", new $class($this));
		}
	}
