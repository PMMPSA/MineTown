...
use onebone\rebirthcoinapi\task\MySQLPingTask;
...
class MySQLProvider implements Provider{
	 ...
	 * @return bool
	 */
	public function createAccount($player, $defaultRebirthCoin = 1000.0){
		if($player instanceof Player){
			$player = $player->getName();
		}
		$player = strtolower($player);
		...
		if(!$this->accountExists($player)){
			$this->db->query("INSERT INTO user_point (username, rebirthcoin) VALUES ('".$this->db->real_escape_string($player)."', $defaultRebirthCoin);");
			return true;
		}
		return false;
	}
